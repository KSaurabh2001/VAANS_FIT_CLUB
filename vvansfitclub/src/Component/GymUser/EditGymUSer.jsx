...
import { editUserProfile } from "../../Redux/GymUser/Action";
// import { editGymUserProfile } from "../../Redux/GymUser/Action"; // Add this when ready

const EditGymUserProfile = () => {
  ...
  useEffect(() => {
    if (user) {
      setFormData((prev) => ({
        ...prev,
        ...user,
        password: "", // clear password for safety
      }));
    }
  }, [user]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log("Edit Gym User Profile Data:", formData);
     dispatch(editUserProfile(formData,token));
      navigate(-1);
    } catch (err) {
  ...
  return (
    <div className="relative min-h-screen bg-black flex items-center justify-center px-4 py-10">
      ...
      <div className="relative z-10 w-full max-w-4xl bg-white/10 backdrop-blur-md text-white p-10 rounded-3xl shadow-xl border border-white/20">
        ...
        <h2 className="text-4xl font-bold mb-10 text-center">Edit Profile – Gym User</h2>
...
export default EditGymUserProfile;
...
//           <div className="flex flex-col">
//             <label className="mb-1 font-semibold">paymentStatus</label>
//             <input
//               value={formData.paymentStatus}
//               disabled
//               className="w-full px-4 py-3 rounded-lg bg-white/20 text-white cursor-not-allowed border border-white/30"
