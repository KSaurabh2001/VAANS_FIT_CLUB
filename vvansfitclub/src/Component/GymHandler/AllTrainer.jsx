...
import UserCard from "../GymOwner/UserCard";

import { useDispatch } from "react-redux";
import { useSelector } from "react-redux";
import { allTrainer } from "../../Redux/GymOwner/Action";

const AllGymTrainers = () => {
  const navigate = useNavigate();
  const [search, setSearch] = useState("");
  const dispatch =useDispatch();
  const token=localStorage.getItem("token");
  const trainer=useSelector((store) => store.owner.AllGymTrainer);

  useEffect(() => {
 dispatch(allTrainer(token));
  },[token,dispatch])

  const filteredTrainers = trainer?.filter((trainer) =>
    `${trainer.name} ${trainer.username}`.toLowerCase().includes(search.toLowerCase())
  );
