...
import { editGym,getGymById } from "../../Redux/GymOwner/Action";
...
const EditGymForm = () => {
  ...
  const gymData = useSelector((store) => store.owner.GymById);
  ...
  // 👇 Set fetched gym data to local form state
  useEffect(() => {
    if (gymData) {
      setFormData({
        id: gymData.id,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await dispatch(editGym(formData, token));
      navigate(-1);
    } catch (error) {
